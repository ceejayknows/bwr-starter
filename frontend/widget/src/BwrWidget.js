...
export default function BwrWidget({ product, merchantDomain, apiBase }) {
  ...
  // When user selects Auto-purchase, ask backend for a SetupIntent
  radioAuto.addEventListener('change', async () => {
    ...
    if (!optRes.ok) {
      status.textContent = optData.error || 'Error';
      return;
    }
    const waitlistId = optData.waitlistId;

    // 2) Ask server for a SetupIntent
    ...
    // 3) Mount Stripe Elements card input
    ...
    try {
      if (!wantsAutopurchase) {
        ...
        status.innerHTML = linkData.invoiceUrl
          ...
          : 'You will get a link on restock.';
        return;
      }

      // Auto-purchase: confirm card setup
